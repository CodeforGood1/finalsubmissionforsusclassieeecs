...
### If Setup Says A Port Is Already In Use

The setup script checks ports before Docker starts and prints the process using the blocked port. This is the usual fix for a non-technical operator:

1. Close the other app using that port. If unsure, restart Docker Desktop and try setup again.
2. If the port is still blocked, open `.env` in Notepad/TextEdit and change the matching `HOST_*_PORT` value.
3. If PostgreSQL port changes, also update `DATABASE_URL` to use the same host port.
4. Run the same setup command again.

Common examples:

```env
HOST_POSTGRES_PORT=15432
DATABASE_URL=postgresql://lms_user:<generated-db-password>@localhost:15432/lms_db
HOST_BACKEND_PORT=5500
HOST_HTTP_PORT=8088
HOST_JITSI_HTTPS_PORT=9443
```

For the default local stack, database-only setup uses `HOST_POSTGRES_PORT`. With `-StartApp` / `--start-app`, local setup also needs SMTP, MailHog, and Jitsi ports. In the full and production Compose stacks, PostgreSQL and SMTP stay internal to Docker; only the app/web, MailHog web UI, and Jitsi ports are published.
...
JICOFO_AUTH_PASSWORD=<generated by setup>
JICOFO_COMPONENT_SECRET=<generated by setup>
JVB_AUTH_PASSWORD=<generated by setup>
HOST_POSTGRES_PORT=5432
HOST_SMTP_PORT=1025
HOST_MAILHOG_WEB_PORT=8025
HOST_BACKEND_PORT=5000
HOST_HTTP_PORT=80
HOST_JITSI_HTTPS_PORT=8443
HOST_JITSI_JVB_UDP_PORT=10000
HOST_JITSI_COLIBRI_PORT=4443
PORT=5000
NODE_ENV=development
DATABASE_URL=postgresql://lms_user:<generated-db-password>@localhost:5432/lms_db
DB_SSL=false
UPLOAD_DIR=./uploads
...
docker run -d --name lms-db `
  ...
  -e POSTGRES_PASSWORD=<generated-db-password> `
...
The smoke test covers health, metrics, admin login, the admin password change endpoint, CSV bulk uploads, MCQ test creation, student submission, and restores the admin password before exiting. If the admin password has already been changed locally, pass it with `-AdminPassword` so the script can log in and restore it.
...
backups/database/                Docker full/prod stack
```

For local `node server.js` usage, install the PostgreSQL client tools so `pg_dump` is available on `PATH`. The Docker runtime image already includes `postgresql-client`.
...
```powershell
curl -H "Authorization: Bearer <admin-token>" http://localhost:5000/api/admin/backups/status
